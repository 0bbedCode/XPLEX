...
import static com.obbedcode.shared.usage.ProcessApi.PROC_COMBINE;
import static com.obbedcode.shared.usage.ProcessApi.PROC_OUT_FLOAT;
import static com.obbedcode.shared.usage.ProcessApi.PROC_OUT_LONG;
import static com.obbedcode.shared.usage.ProcessApi.PROC_OUT_STRING;
import static com.obbedcode.shared.usage.ProcessApi.PROC_PARENS;
import static com.obbedcode.shared.usage.ProcessApi.PROC_SPACE_TERM;
...
public class ProcessCpuTracker {
    ...
    private void updateLocked() {
        ...
        if (ProcessApi.readProcFile("/proc/stat", SYSTEM_CPU_FORMAT,
                null, sysCpu, null)) {
        ...
        if (ProcessApi.readProcFile("/proc/loadavg", LOAD_AVERAGE_FORMAT,
    ...
    private int[] collectStats(String statsFile, int parentPid, boolean first,
        ...
        int[] pids = ProcessApi.getPids(statsFile, curPids);
        ...
        for (int i=0; i<NP; i++) {
            ...
            if (st != null && st.pid == pid) {
                ...
                if (st.interesting) {
                    ...
                    if (!ProcessApi.readProcFile(st.statFile.toString(),
            ...
            if (st == null || st.pid > pid) {
                ...
                if (ProcessApi.readProcFile(path, PROCESS_FULL_STATS_FORMAT, procStatsString,
                        procStats, null)) {
    ...
    public long getCpuTimeForPid(int pid) {
        ...
        if (ProcessApi.readProcFile(statFile, PROCESS_STATS_FORMAT,
                null, statsData, null)) {
    ...
    public long getCpuDelayTimeForPid(int pid) {
        ...
        final long[] statsData = new long[4];
        if (ProcessApi.readProcFile(statFile, PROCESS_SCHEDSTATS_FORMAT,
                null, statsData, null)) {
